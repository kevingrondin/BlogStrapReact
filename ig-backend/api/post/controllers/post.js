...
module.exports = {
  async create(ctx) {
    ...
    if (ctx.is("multipart")) {
      ...
      if (!data || !data.description) ctx.throw(400, "Add some content");
      if (!files || !files.image) ctx.throw(400, "Add a file");
  ...
  async update(ctx) {
    ...
    let entity;
    if (ctx.is("multipart")) {
      ctx.throw(400, "Please only make JSON request an updated description");
    } else {
